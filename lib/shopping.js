...
// Marca de categoria que o backend usa para o que o PACIENTE acrescentou à
// lista (não é uma gôndola). Espelha AISLE.EXTRA da API.
export const CATEGORIA_EXTRA = "Extra do paciente";

// Igual a categorizeShopping, mas para itens vindos da API: preserva o objeto
// inteiro ({ id, name, isChecked, … }) em vez de reduzir a nome, porque o
// checklist precisa do id para persistir a marcação. Itens marcados como extra
// do paciente ficam de fora — eles têm seção própria na tela.
export function categorizeShoppingItems(items) {
  const groups = new Map();
  (items || []).forEach((item) => {
    if (!item || !item.name || !item.name.trim()) return;
    if (item.category === CATEGORIA_EXTRA) return;
    const cat = classify(item.name);
    if (!groups.has(cat.group)) groups.set(cat.group, { group: cat.group, icon: cat.icon, items: [] });
    groups.get(cat.group).items.push(item);
  });
  groups.forEach((g) => g.items.sort((a, b) => a.name.localeCompare(b.name, "pt-BR")));
  return ORDER.map((o) => groups.get(o)).filter(Boolean);
}

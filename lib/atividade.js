...
export function montaDiario(dp, logs, extras, hoje = new Date()) {
  ...
  (logs || []).forEach((l) => {
    ...
    if (l.status === "consumed") acc.consumidos += 1;

    // Todo registro vira evento, inclusive o que ela comeu: o diário conta o
    // dia dela por inteiro, não só o que fugiu do plano.
    const marca =
      l.status === "consumed"
        ? { ate: true }
        : l.status === "swapped"
        ? { to: l.swappedFoodName || "outro alimento" }
        : { skip: true };
    acc.events.push({
  ...
  return diasDoDiario(dp, hoje).map((d) => {
    ...
    // Na ordem das refeições do dia; o que não tem horário (extras) vai ao fim.
    const events = acc ? acc.events.slice().sort((a, b) => (a.time || "99").localeCompare(b.time || "99")) : [];
    return {
      label: DIA_SEMANA[d.getDay()],
      date: ddmm(d),
      isToday: dia === hojeIso,
      adesao,
      registros: acc ? acc.registrados : 0,
      events,
    };
  });
}
